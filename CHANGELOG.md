...
- Znany osobny problem (do fixu za zgodą): pętla retry pushu w `scanner.yml` nie robi `git rebase --abort` po konflikcie — run 22:05 (wyścig z równoległym runem) stracił wyniki scanu z komunikatem `Push failed after 3 attempts`.
